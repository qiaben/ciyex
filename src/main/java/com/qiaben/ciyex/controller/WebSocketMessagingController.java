...
public class WebSocketMessagingController {
     ...
     */
    public void notifyNewMessage(CommunicationDto message, Long orgId) {
        log.info("New message notification triggered for org {} - message ID: {}", orgId, message.getId());
     ...
     * Currently using polling-based approach instead of WebSocket
     */
    public void notifyMessageRead(CommunicationDto message, Long orgId) {
        log.info("Message read notification triggered for org {} - message ID: {}", orgId, message.getId());
