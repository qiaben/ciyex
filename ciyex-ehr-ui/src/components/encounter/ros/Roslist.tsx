...
//
// // Safe JSON (handles empty body/401/204)
...
//         setShowForm(false);
//         setEditing(null);
//         await load(); // keep UI in sync with backend
//     }
//
...
//             if (res.status === 204) {
//                 setItems((p) => p.filter((x) => x.id !== id));
//                 setAlert({ type: "success", msg: "ROS entry deleted." });
//                 return;
//             }
//
//             const json = await safeJson<ApiResponse<void>>(res);
...
//
//             setAlert({ type: "success", msg: "ROS entry e-signed." });
...
//     function printROS(ros: RosDto) {
//         try {
//             const win = window.open("", "_blank", "noopener,noreferrer");
//             if (!win) throw new Error("Popup blocked. Please allow popups to print.");
//
//             const created = ros.audit?.createdDate ? `Created: ${ros.audit.createdDate}` : "";
//             const updated = ros.audit?.lastModifiedDate ? ` · Updated: ${ros.audit.lastModifiedDate}` : "";
//
//             win.document.write(`<!doctype html>
// <html>
// <head>
// <meta charset="utf-8" />
// <title>ROS #${ros.id ?? ""}</title>
// <style>
...
//   <div class="meta">Patient #${patientId} · Encounter #${encounterId} · ID ${ros.id ?? ""} ${created}${updated}</div>
//   <div class="card">
//     <div class="row"><strong>System:</strong> ${escapeHtml(ros.system || "—")}</div>
//     <div class="row"><strong>Status:</strong> ${escapeHtml(ros.status || "—")}</div>
//     ${ros.finding ? `<div class="row"><strong>Findings:</strong> ${escapeHtml(ros.finding)}</div>` : ""}
//     ${ros.notes ? `<div class="row"><strong>Notes:</strong><br/>${escapeHtml(ros.notes).replace(/\\n/g,"<br/>")}</div>` : ""}
//   </div>
