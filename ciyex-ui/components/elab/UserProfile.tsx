...
const UserProfile: React.FC = () => {
  ...
  // Count all individual test results with status 'scheduled'
  const upcomingTestsCount = orders.reduce((acc, order) => {
    return acc + (order.results?.filter(r => r.status?.toLowerCase() === 'scheduled').length || 0);
  }, 0);
