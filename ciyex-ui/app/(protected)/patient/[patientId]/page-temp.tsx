...
import { getCurrentUserFromToken } from "../../../utils/auth"; // Corrected import

import {
    User, Calendar, Phone, MapPin, Droplet, Heart, Users, Edit,
    FileText, Receipt, FlaskConical, Home
} from "lucide-react";

interface ParamsProps {
    params: Promise<{ patientId: string }>;
    searchParams?: Promise<{ [key: string]: string | string[] | undefined }>;
    user?: any; // JWT user object from parent/server
}
...
const PatientProfile = async (props: ParamsProps) => {
    ...
    // Use JWT user if patientId is "self"
    if (patientId === "self") {
        const user = await getCurrentUserFromToken(); // Fetch current user from JWT token
        id = user?.userId?.toString() || ""; // Convert userId to string, fallback to an empty string if undefined
    } else {
        id = patientId;
    }

    if (!id) {
        return <div>Unable to determine patient id.</div>;
    }
