...
import { getCurrentUserFromToken } from "../../utils/auth"; // <-- Correct import, assuming tsconfig has @/utils
...
const PatientDashboard = async () => {
  // Reads token from JWT_SECRET env var, parses as described earlier
  const user = await getCurrentUserFromToken();

  if (!user) {
    // No user found, redirect to sign-in or show an error
    redirect("/sign-in");
  }
  ...
  const {
    ...
    availableDoctor = [],
    monthlyData = [],
  } = (await getPatientDashboardStatistics(String(user.userId))) ?? {};
    ...
    return (
        <div className="min-h-screen flex items-center justify-center">
          <div className="bg-white p-8 rounded shadow text-center">
            ...
            <p className="mb-4">
              We couldn't find your patient profile. Please complete your registration or contact support if you believe this is an error.
            </p>
            <Button asChild>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-4 xl:gap-8">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl xl:text-4xl font-extrabold text-foreground tracking-tight leading-tight">
                      Welcome, {data?.first_name || user?.fullName}
                    </h1>
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-4 items-stretch">
            <div className="w-full h-full flex flex-col gap-4">
              ...
              <div className="animate-fadeInUp transition-all duration-300 hover:scale-[1.02]">
                <PatientRatingContainer id={user?.userId?.toString()} />
              </div>
            </div>
          </div>
        </div>
