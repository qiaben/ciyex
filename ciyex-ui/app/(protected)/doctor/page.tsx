...
const DoctorDashboard = async () => {
  // Get the current user using JWT (from your helper)
  const user = await getCurrentUserFromToken();

  // Add status/role check
  // Assume "roles" is an array, pick first or find "doctor"
  const role = user?.roles?.includes("DOCTOR") ? "doctor" : user?.roles?.[0]?.toLowerCase();
  // If you store status elsewhere, adjust as needed
  // If your user object includes a status, e.g. user.status:
  ...
  const {
    ...
    totalRevenue,
    monthlyRevenueData,
  } = await getDoctorDashboardStats(String(user?.userId));

  // Get ratings for the current doctor
  const ratings = await getDoctorRatings(String(user?.userId));
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="mb-6">
                <h1 className="text-3xl md:text-4xl font-extrabold text-[#1e293b] dark:text-white drop-shadow-sm font-sans tracking-tight">
                  <span className="font-semibold text-[#10b981] dark:text-[#5eead4]">{greeting},</span> <span className="font-extrabold text-[#2563eb] dark:text-[#38bdf8]">Dr. {user?.fullName}</span>
                </h1>
              ...
              >
                <Link href={`/record/doctors/${user?.userId}`}>
