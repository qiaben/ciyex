...
const AppointmentSchedule: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("Error fetching appointments:", error instanceof Error ? error.message : 'Unknown error occurred');
        setAppointments([]);
      }
    })();
  ...
  const joinAppointment = (appointment: Appointment) => {
    router.push(`/doctor/video-call?appointmentId=${appointment.id}`);
  };
